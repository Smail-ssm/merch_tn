...
type RestOf<T extends IUsers | NewUsers> = Omit<T, 'createdAt' | 'updatedAt' | 'date'> & {
...
export class UsersService {
  ...
  protected convertDateFromClient<T extends IUsers | NewUsers | PartialUpdateUsers>(users: T): RestOf<T> {
    return {
      ...users,
      createdAt: users.createdAt?.format(DATE_FORMAT) ?? null,
      updatedAt: users.updatedAt?.format(DATE_FORMAT) ?? null,
      date: users.date?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restUsers: RestUsers): IUsers {
    return {
      ...
      date: restUsers.date ? dayjs(restUsers.date) : undefined,
    };
  }
