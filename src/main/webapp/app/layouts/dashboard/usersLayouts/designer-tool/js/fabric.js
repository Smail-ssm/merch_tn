...
Cufon.registerEngine(
  ...
  (function () {
    ...
    var HAS_BROKEN_LINEHEIGHT =
      !HAS_INLINE_BLOCK && (fabric.document.compatMode == 'BackCompat' || /frameset|transitional/i.test(fabric.document.doctype.publicId));
...
(function () {
  ...
  function groupSVGElements(elements, options, path) {
    var object = elements.length > 1 ? new fabric.PathGroup(elements, options) : elements[0];
...
(function (global) {
  ...
  function pointerX(event) {
    ...
    return (
      event.pageX ||
      (typeof event.clientX != 'unknown' ? event.clientX : 0) + (docElement.scrollLeft || body.scrollLeft) - (docElement.clientLeft || 0)
    );
  }
  ...
  function pointerY(event) {
    ...
    return (
      event.pageY ||
      (typeof event.clientY != 'unknown' ? event.clientY : 0) + (docElement.scrollTop || body.scrollTop) - (docElement.clientTop || 0)
    );
  }
...
(function (global) {
  ...
  fabric.parseSVGDocument = (function () {
    ...
    var reViewBoxAttrValue = new RegExp(
      '^' + '\\s*(' + reNum + '+)\\s*,?' + '\\s*(' + reNum + '+)\\s*,?' + '\\s*(' + reNum + '+)\\s*,?' + '\\s*(' + reNum + '+)\\s*' + '$',
    );
  ...
  fabric.Gradient = fabric.util.createClass(
    /** @scope fabric.Gradient.prototype */ {
      ...
      toLiveGradient: function (ctx) {
        var gradient = ctx.createLinearGradient(this.x1, this.y1, this.x2 || ctx.canvas.width, this.y2);
  ...
  var InteractiveMethods = /** @scope fabric.Canvas.prototype */ {
    ...
    _setCursorFromEvent: function (e, target) {
      ...
      if (!target) {
        ...
        var corner =
          !!target._findTargetCorner && (!activeGroup || !activeGroup.contains(target)) && target._findTargetCorner(e, this._offset);
    ...
    _drawSelection: function () {
      ...
      this.contextTop.fillRect(groupSelector.ex - (left > 0 ? 0 : -left), groupSelector.ey - (top > 0 ? 0 : -top), aleft, atop);
  ...
  fabric.Object = fabric.util.createClass(
    /** @scope fabric.Object.prototype */ {
      ...
      _set: function (key, value) {
        var shouldConstrainValue = (key === 'scaleX' || key === 'scaleY') && value < fabric.Object.MIN_SCALE_LIMIT;
      ...
      render: function (ctx, noTransform) {
        ...
        if (this.overlayFill) {
          ...
          ctx.fillStyle = this.fill.toLiveGradient ? this.fill.toLiveGradient(ctx) : this.fill;
        }
  ...
  fabric.Group = fabric.util.createClass(
    ...
    /** @scope fabric.Group.prototype */ {
      ...
      containsPoint: function (point) {
        ...
        return (
          centerX - halfWidth < point.x && centerX + halfWidth > point.x && centerY - halfHeight < point.y && centerY + halfHeight > point.y
        );
      },
  ...
  fabric.Image = fabric.util.createClass(
    ...
    /** @scope fabric.Image.prototype */ {
      ...
      _setWidthHeight: function (options) {
        ...
        this.width = 'width' in options ? options.width : (this.getElement().width || 0) + sidesBorderWidth;
...
(function (global) {
  ...
  fabric.Text = fabric.util.createClass(
    ...
    /** @scope fabric.Text.prototype */ {
      ...
      _renderTextFill: function (ctx, textLines) {
        this._boundaries = [];
        for (var i = 0, len = textLines.length; i < len; i++) {
          ctx.fillText(textLines[i], -this.width / 2, -this.height / 2 + i * this.fontSize * this.lineHeight);
        }
      },
      ...
      _renderTextStroke: function (ctx, textLines) {
        if (this.strokeStyle) {
          for (var i = 0, len = textLines.length; i < len; i++) {
            ctx.strokeText(textLines[i], -this.width / 2, -this.height / 2 + i * this.fontSize * this.lineHeight);
          }
        }
      },
      ...
      _renderTextDecoration: function (ctx, textLines) {
        ...
        function renderLinesAtOffset(offset) {
          for (var i = 0, len = textLines.length; i < len; i++) {
            ...
            ctx.fillRect(-this.width / 2 + lineLeftOffset, offset + i * this.fontSize * this.lineHeight - halfOfVerticalBox, lineWidth, 1);
          }
        }
  ...
  fabric.Text.ATTRIBUTE_NAMES = (
    'x y fill fill-opacity opacity stroke stroke-width transform ' + 'font-family font-style font-weight font-size text-decoration'
  ).split(' ');
