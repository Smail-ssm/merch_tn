...
import com.xdev.merch.repository.RolesRepository;
import java.net.URI;
...
/**
 * REST controller for managing {@link com.xdev.merch.entities.Roles}.
 */
@RestController
    ...
    public ResponseEntity<Roles> partialUpdateRoles(@PathVariable(value = "id", required = false) final Long id, @RequestBody Roles roles)
        ...
        Optional<Roles> result = rolesRepository
            ...
            .map(existingRoles -> {
                ...
                if (roles.getSection() != null) {
                    existingRoles.setSection(roles.getSection());
                }
